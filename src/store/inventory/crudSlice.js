...
export const getIndexEntityData = createAsyncThunk(
	...
	async (value, { rejectWithValue }) => {
		try {
			const data = await getDataWithParam(value);
			return data;
		} catch (error) {
...
export const getProductSkuItemIndexEntityData = createAsyncThunk(
	"index-sku-item",
	async (value) => {
		try {
...
export const showInstantEntityData = createAsyncThunk(
	...
	async (value, { rejectWithValue }) => {
		try {
			const data = await showData(value);
			return data;
		} catch (error) {
...
const crudSlice = createSlice({
	...
	extraReducers: (builder) => {
		...
		builder.addCase(updateEntityDataWithFile.fulfilled, () => {
			// state.entityDataDelete = action.payload.data.message
		});
	},
});
