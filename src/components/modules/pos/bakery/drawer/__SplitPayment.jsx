...
import { IconDeviceFloppy, IconRefreshDot, IconUserCircle, IconX } from "@tabler/icons-react";
	...
	useEffect(() => {
		if (currentSplitPayments && currentSplitPayments.length > 0) {
			...
			currentSplitPayments.forEach((payment) => {
				values[`partial_amount_${payment.transaction_mode_id}`] = payment.partial_amount;
				values[`remarks_${payment.transaction_mode_id}`] = payment.remarks || "";
			});
