...
function __PurchaseForm(props) {
	...
	const entityNewData = useSelector((state) => state.crudSlice.data.purchase?.newData);
	...
	useEffect(() => {
		if (vendorData) {
			window.dbAPI.getDataFromTable("core_vendors", vendorData).then((data) => {
				if (data?.length > 0) {
					setVendorObject(data[0]);
				}
			});
		}
	}, [vendorData]);
								...
								<Grid gutter={{ base: 4 }}>
									<Grid.Col span={3}>
										<Center fz={"md"} fw={"800"}>
											{currencySymbol}{" "}
											{purchaseSubTotalAmount &&
												Number(purchaseSubTotalAmount).toFixed(2)}
										</Center>
									</Grid.Col>
									<Grid.Col span={3}>
										<Center fz={"md"} fw={"800"}>
							...
							<ScrollArea h={formHeight} scrollbarSize={2} type="never" bg={"gray.1"}>
								...
								>
									<Grid gutter={{ base: 2 }}>
										...
										<Grid.Col span={6}>
											<Box ta="right" fz={"md"} mt={"4"} c={"red"} fw={"800"}>
												{returnOrDueText} {currencySymbol}{" "}
												{purchaseDueAmount.toFixed(2)}
											</Box>
										</Grid.Col>
									</Grid>
									<Box mt={"xs"} h={1} bg={`red.3`} />
