...
import { Grid, Box, Center, Group, Flex, Stack, Text } from "@mantine/core";
...
export function PurchasePrintNormal(props) {
	...
	useEffect(() => {
		!effectRan.current && (handlePrint(), setPrintA4(false), (effectRan.current = true));
	}, []);
							...
							<Box>
								<Group justify="center" gap="xs">
									...
									<Stack align="flex-start" justify="center" gap="0">
										...
										<Text h={18} fw={"300"} fz={"12"}>
											{purchaseViewData &&
												purchaseViewData.createdByName &&
												purchaseViewData.createdByName}
										</Text>
										<Text h={18} fw={"300"} fz={"12"}>
											{purchaseViewData &&
						...
						<table className={classes["invoice-body-table"]}>
							<thead>
								<tr className={classes["invoice-body-table-tr"]}>
									...
									<th className={classes["invoice-body-table-th"]}>
										{t("SalesPrice")}
									</th>
									<th
										className={`${classes["invoice-body-table-th"]} ${classes["text-right"]}`}
								...
								<div className={classes["invoice-footer-two-right"]}>
									<p className={classes["invoice-footer-text-two"]}>
										{purchaseViewData &&
											...
											Number(purchaseViewData.sub_total).toFixed(2)}
									</p>
									<p className={classes["invoice-footer-text-two"]}>
										{purchaseViewData &&
											purchaseViewData.discount &&
											Number(purchaseViewData.discount).toFixed(2)}
									</p>
									<p className={classes["invoice-footer-text-two"]}>
										{purchaseViewData &&
											purchaseViewData.total &&
											Number(purchaseViewData.total).toFixed(2)}
									</p>
									<p
										className={`${classes["invoice-footer-text-two"]} ${classes["footer-border-bottom"]}`}
									>
										{purchaseViewData &&
											...
											Number(purchaseViewData.payment).toFixed(2)}
									</p>
									<p className={classes["invoice-footer-text-two"]}>
										{purchaseViewData &&
											purchaseViewData.total &&
											Number(
												purchaseViewData.total - purchaseViewData.payment
											).toFixed(2)}
									</p>
